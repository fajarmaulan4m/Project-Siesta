...
BEATPORT_SEMAPHORE = asyncio.Semaphore(5)
...
async def download_beatport_track(url: str, filepath: str, proxy: str = None):
    try:
        ...
        return None 
    except Exception as e: return f"Gagal download: {e}"
...
async def start_track(item_id: str, user: dict, track_meta: dict | None, upload=True, filepath=None, disable_link=False):
    async with BEATPORT_SEMAPHORE:
        ...
        err = await download_beatport_track(track_meta['download_url'], filepath, proxy=user_proxy)
        if err == "403_FORBIDDEN":
            ...
            if new_url:
                ...
                err = await download_beatport_track(new_url, filepath, proxy=user_proxy)
        ...
        try:
            ...
            if cover_src:
                ...
                if os.path.exists(cover_src): shutil.copy(cover_src, target)
                    ...
                    u_proxy = beatport_manager.get_client(user.get('user_id')).proxy
                    await download_beatport_track(cover_src, target, proxy=u_proxy)
        except: pass
